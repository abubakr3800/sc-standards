#!/usr/bin/env python3
"""
Optimized Dialux PDF Processor
Final optimized version that properly handles real Dialux reports with multiple rooms
"""
import re
import json
...
@dataclass
class OptimizedRoom:
    """Optimized room data with consolidated parameters"""
    name: str
    area: float
...
class OptimizedReport:
    """Optimized Dialux report with consolidated analysis"""
    project_name: str
    project_type: str
    total_rooms: int
    total_area: float
...
class OptimizedDialuxProcessor:
    """Optimized processor that properly handles real Dialux reports with multiple rooms"""
    ...
    def extract_optimized_data(self, text: str) -> List[Dict[str, Any]]:
        """Extract lighting data with optimized approach"""
        ...
        # Group parameters by context/room with better logic
        grouped_data = self._group_parameters_optimized(all_parameters, text)
        print(f"🏠 Grouped into {len(grouped_data)} potential rooms")
        
        # Consolidate and validate groups
        consolidated_data = self._consolidate_groups_optimized(grouped_data)
                    ...
                    try:
                        ...
                        if self._is_reasonable_value(param_name, value):
                            param_data = {
                                ...
                                'text_around': text[max(0, match.start()-100):match.end()+100]
                            }
    ...
    def _group_parameters_optimized(self, parameters: List[Dict[str, Any]], text: str) -> List[Dict[str, Any]]:
        """Group parameters by context with optimized logic"""
        groups = []
        
        # Sort parameters by position
        parameters.sort(key=lambda x: x['position'])
        
        # Group nearby parameters with better logic
        current_group = []
        last_position = -1000
        
        for param in parameters:
            # If parameter is far from the last one, start a new group
            if param['position'] - last_position > 300:  # Increased distance
                if current_group:
        ...
        # Convert groups to room data with better filtering
        room_data = []
        for i, group in enumerate(groups):
            if len(group) >= 1:  # At least 1 parameter to form a room
                room_name = self._extract_room_name_optimized(group, text)
                room_params = {}
    ...
    def _extract_room_name_optimized(self, group: List[Dict[str, Any]], text: str) -> str:
        """Extract room name from a group of parameters with optimized logic"""
        # Get the context around the first parameter
        first_param = group[0]
        context_start = max(0, first_param['position'] - 200)
        context_end = min(len(text), first_param['position'] + 200)
        context = text[context_start:context_end]
        
        # Look for room names in the context with better patterns
        room_patterns = [
        ...
        # If no specific room name found, create a descriptive one
        param_types = [p['parameter'] for p in group]
        if 'illuminance_avg' in param_types:
            return f"Lighting Area {len(group)}"
        elif 'area' in param_types:
            return f"Space {len(group)}"
        else:
            return f"Room {len(group)}"
    
    def _consolidate_groups_optimized(self, groups: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Consolidate groups and remove duplicates with optimized logic"""
        consolidated = {}
        ...
        # Convert back to list with better filtering
        result = []
    ...
    def create_optimized_rooms(self, lighting_data: List[Dict[str, Any]]) -> List[OptimizedRoom]:
        """Create optimized room objects with consolidated data"""
        rooms = []
        
        print(f"🏠 Creating optimized rooms from {len(lighting_data)} consolidated data points")
        ...
        for room_data in lighting_data:
            try:
                ...
                # Include rooms with at least 1 core parameter
                if data_completeness > 0.1:  # At least 10% of core parameters
                    ...
                    rooms.append(room)
                    print(f"✅ Created optimized room: {room_name}")
    ...
    def _calculate_compliance(self, room: OptimizedRoom) -> OptimizedRoom:
    ...
    def process_optimized_dialux_pdf(self, pdf_path: Path) -> Optional[OptimizedReport]:
        """Process a Dialux PDF with optimized analysis"""
        try:
            print(f"⚡ Optimized processing of Dialux PDF: {pdf_path.name}")
            ...
            lighting_data = self.extract_optimized_data(text)
            ...
            # Create optimized rooms
            rooms = self.create_optimized_rooms(lighting_data)
            print(f"🏠 Created {len(rooms)} optimized rooms")
            ...
            print(f"✅ Successfully created optimized report with {len(rooms)} rooms")
            return report
    ...
    def _determine_project_type(self, rooms: List[OptimizedRoom]) -> str:
    ...
    def _calculate_standard_compliance(self, rooms: List[OptimizedRoom], standard_name: str) -> float:
...
def test_optimized_processor():
    ...
    for test_file in test_files:
        ...
        if pdf_path.exists():
            print(f"\n🧪 Testing optimized processor with: {pdf_path.name}")
            result = processor.process_optimized_dialux_pdf(pdf_path)
            if result:
